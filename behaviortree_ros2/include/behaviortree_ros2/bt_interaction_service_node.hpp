...
// Copied from bt_service_node.hpp - TODO do the inheritence better - Greg
 ...
 *
 * class AddTwoNumbers: public RosInteractionServiceNode<example_interfaces::srv::AddTwoInts>
 *
 * RosInteractionServiceNode will try to be non-blocking for the entire duration of the call.
...
class RosInteractionServiceNode : public BT::InteractionNode
{
...
public:
   ...
   */
  explicit RosInteractionServiceNode(const std::string & instance_name,
                          const BT::NodeConfig& conf,
                          const BT::RosNodeParams& params);

  virtual ~RosInteractionServiceNode() = default;

  /**
   * @brief Any subclass of RosInteractionServiceNode that has ports must implement a
   ...
   * RosInteractionServiceNode::onFailure(INVALID_REQUEST) will be called.
   */
...
template<class T> inline
  RosInteractionServiceNode<T>::RosInteractionServiceNode(const std::string & instance_name,
                                    const NodeConfig &conf,
                                    const RosNodeParams& params):
  BT::InteractionNode(instance_name, conf),
  node_(params.nh),
...
template<class T> inline
  bool RosInteractionServiceNode<T>::createClient(const std::string& service_name)
{
...
template<class T> inline
  NodeStatus RosInteractionServiceNode<T>::tick()
{
  ...
  auto CheckStatus = [](NodeStatus status)
  {
    ...
    {
      throw std::logic_error("RosInteractionServiceNode: the callback must return either SUCCESS or FAILURE");
    }
    return status;
  };
...
template<class T> inline
  void RosInteractionServiceNode<T>::halt()
{
  if( status() == NodeStatus::RUNNING )
  {
    resetStatus();
