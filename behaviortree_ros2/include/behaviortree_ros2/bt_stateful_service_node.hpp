 ...
 *
 * class AddTwoNumbers: public RosStatefulServiceNode<example_interfaces::srv::AddTwoInts>
 *
 * RosStatefulServiceNode will try to be non-blocking for the entire duration of the call.
...
class RosStatefulServiceNode : public BT::StatefulActionNode
{
public:
  // Type definitions
   ...
   */
  explicit RosStatefulServiceNode(const std::string& instance_name, const BT::NodeConfig& conf,
                          const BT::RosNodeParams& params);

  virtual ~RosStatefulServiceNode() = default;

  /**
   * @brief Any subclass of RosStatefulServiceNode that has ports must implement a
   ...
   * RosStatefulServiceNode::onFailure(INVALID_REQUEST) will be called.
   */
...
protected:
  ...
  rclcpp::Logger logger()
  {
    ...
    return rclcpp::get_logger("RosStatefulServiceNode");
  }
...
template <class T>
inline RosStatefulServiceNode<T>::ServiceClientInstance::ServiceClientInstance(
...
template <class T>
inline RosStatefulServiceNode<T>::RosStatefulServiceNode(const std::string& instance_name,
...
template <class T>
inline bool RosStatefulServiceNode<T>::createClient(const std::string& service_name)
{
...
template <class T>
inline void RosStatefulServiceNode<T>::setServiceName(const std::string& service_name)
{
  service_name_ = service_name;
...
template <class T>
inline NodeStatus RosStatefulServiceNode<T>::onStart()
{
  if(!rclcpp::ok())
  {
    halt();
  ...
  if(!srv_instance_)
  {
    throw BT::RuntimeError("RosStatefulServiceNode: no service client was specified neither as "
  ...
  auto CheckStatus = [](NodeStatus status) {
    ...
    {
      throw LogicError("RosStatefulServiceNode: the callback must return either SUCCESS nor "
                       "FAILURE");
    }
    return status;
  };
...
template <class T>
inline NodeStatus RosStatefulServiceNode<T>::onRunning()
{
  auto CheckStatus = [](NodeStatus status) {
    if(!isStatusCompleted(status))
    {
...
template <class T>
inline void RosStatefulServiceNode<T>::onHalted()
{
  if(status() == NodeStatus::RUNNING)
  {
    resetStatus();
